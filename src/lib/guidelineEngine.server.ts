...
export function getRecommendations(profile: UserProfile, history: ScreeningEvent[]): Recommendation[] {
  ...
  // Age 45-75
  if (age >= 45 && age <= 75) {
    const lastColonoscopy = history.find(e => e.type === 'colonoscopy' && e.status === 'completed');
    const lastFit = history.find(e => e.type === 'fit' && e.status === 'completed');

    let status: Recommendation['status'] = 'due_now';
    let reason = "Based on your age (45-75), routine colorectal screening is recommended.";

    if (lastColonoscopy) {
      // Simplistic check for 10 years
      status = 'completed';
      reason = "Prior colonoscopy recorded. Next screening usually in 10 years unless high risk.";
    }
